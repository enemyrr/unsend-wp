...
class UnsendWPMailer_API
{
     ...
     */
    public function send_email($email_data, $temp_api_key = null, $temp_api_url = null)
    {
        $api_key_to_use = $temp_api_key ?: $this->api_key;
        $api_url_to_use = $temp_api_url ?: $this->api_url;
        ...
        $response = wp_remote_post($api_url_to_use, array(
            'headers' => $headers,
    ...
    private function prepare_email_data($email_data)
    {
        ...
        // If no HTML or text provided but we have a message, determine type from content_type
        if (empty($body['html']) && empty($body['text']) && !empty($email_data['message'])) {
            if (isset($email_data['content_type']) && strtolower($email_data['content_type']) === 'text/html') {
                $body['html'] = $email_data['message'];
            } else {
                // Default to text if content_type is not text/html or not set
     ...
     */
    public function test_connection($temp_api_key = null, $temp_api_url = null)
    {
        $test_data = array(
            'to' => get_option('admin_email'),
            // For 'from', use the currently saved plugin settings, not potentially unsaved ones from a form.
            'from' => get_option('unsend_from_email', get_option('admin_email')),
            'subject' => 'Unsend WP Mailer Connection Test', // Changed subject slightly for clarity
            'text' => 'This is a connection test email from the Unsend WP Mailer plugin. If you receive this, the specified API credentials and endpoint are working correctly.',
        );

        return $this->send_email($test_data, $temp_api_key, $temp_api_url);
    }
}